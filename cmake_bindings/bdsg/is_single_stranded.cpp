...
void bind_bdsg_is_single_stranded(std::function< pybind11::module &(std::string const &namespace_) > &M)
{
	...
	// bdsg::algorithms::single_stranded_orientation(const class handlegraph::HandleGraph *) file:bdsg/is_single_stranded.hpp line:32
	M("bdsg::algorithms").def("single_stranded_orientation", (class std::vector<struct handlegraph::handle_t, class std::allocator<struct handlegraph::handle_t> > (*)(const class handlegraph::HandleGraph *)) &bdsg::algorithms::single_stranded_orientation, "Returns a vector of handles where the orientation of each handle indicates an\n orientation that could be used to convert the graph into a single-stranded graph.\n That is, if all of the reverse handles (or all of the forward handles) were swapped\n in orientation, the graph would contain no reversing edges. Returns an empty vector\n if there is no such combination of node orientations (also if graph has no nodes).\n\nC++: bdsg::algorithms::single_stranded_orientation(const class handlegraph::HandleGraph *) --> class std::vector<struct handlegraph::handle_t, class std::allocator<struct handlegraph::handle_t> >", pybind11::arg("graph"));
