...
net_handle_t SnarlDistanceIndex::get_bound(const net_handle_t& snarl, bool get_end, bool face_in) const {
    if (get_handle_type(snarl) == CHAIN_HANDLE) {
        ...
        return get_net_handle(offset, connectivity,  NODE_HANDLE);
...
bool SnarlDistanceIndex::follow_net_edges_impl(const net_handle_t& here, const handlegraph::HandleGraph* graph, bool go_left, const std::function<bool(const net_handle_t&)>& iteratee) const {
    ...
    if (parent_record.get_record_handle_type() == ROOT_HANDLE) {
        ...
        if (get_handle_type(here) == SENTINEL_HANDLE) {
            ...
            graph_handle = graph->get_handle(node_id(here), 
        ...
        graph->follow_edges(graph_handle, false, [&](const handle_t& h) {
            ...
            if (graph->get_id(h) == parent_record.get_start_id()) {
                ...
                NodeRecord next_node_record(get_node_net_handle(graph->get_id(h)), &snarl_tree_records);
...
size_t SnarlDistanceIndex::distance_in_parent(const net_handle_t& parent, 
    ...
    for (CachedNetHandle& cached_child : {cached_child1, cached_child2}) {
        //Fill in necessary information
        SnarlTreeRecord record(cached_child, &snarl_tree_record);
        cached_child.parent_offset = record.get_parent_record_offset();
        cached_child.rank_in_parent = record.get_rank_in_parent();
        cached_child.reversed_in_parent = record.get_is_reversed_in_parent();

        if (get_record_type(cached_child.tag) == DISTANCED_NODE_CHAIN || 
            get_record_type(cached_child.tag) == MULTICOMPONENT_NODE_CHAIN ) {
            cached_child.prefix_sum = NodeRecord(cached_child.net, &snarl_tree_records).get_prefix_sum(); 
            cached_child.forward_loop = NodeRecord(cached_child.net, &snarl_tree_records).get_forward_loop(); 
            cached_child.reverse_loop = NodeRecord(cached_child.net, &snarl_tree_records).get_reverse_loop(); 
            if (get_record_type(cached_child.tag) == MULTICOMPONENT_NODE_CHAIN) {
                cached_child.component = NodeRecord(cached_child.net, &snarl_tree_records).get_chain_component(); 
            }
        }
    }
...
size_t SnarlDistanceIndex::distance_in_parent(const CachedNetHandle& cached_parent, 
        const CachedNetHandle& cached_child1, const CachedNetHandle& cached_child2, const HandleGraph* graph) const {

#ifdef debug_distances
    assert(canonical(parent) == canonical(get_parent(child1)));
    assert(canonical(parent) == canonical(get_parent(child2)));
#endif
    net_handle_t& child1 = cached_child1.net;
    net_handle_t& child2 = cached_child2.net;
    ...
    if (is_root(parent)) {
        ...
        //connected
        size_t parent_record_offset1 = cached_child1.parent_offset == std::numeric_limits<size_t>::max() ? 
                                        SnarlTreeRecord(child1, &snarl_tree_records).get_parent_record_offset()
                                        : cached_child1.parent_offset;
        size_t parent_record_offset2 = cached_child2.parent_offset == std::numeric_limits<size_t>::max() ? 
                                        SnarlTreeRecord(child2, &snarl_tree_records).get_parent_record_offset() :
                                        cached_child2.parent_offset;
        if (parent_record_offset1 != parent_record_offset2) {
            //If the children are in different connected components
            return std::numeric_limits<size_t>::max();
        } else if (get_record_type(cached_parent.tag) != DISTANCED_ROOT_SNARL){
            ...
            if (get_record_offset(child1) == get_record_offset(child2)) {
                ...
                if (ends_at(child1) == START && ends_at(child2) == START) {
                    return has_external_connectivity(cached_child1.tag, START, START) ? 0 : std::numeric_limits<size_t>::max();
                } else if (ends_at(child1) == END && ends_at(child2) == END) {
                    return has_external_connectivity(cached_child1.tag, END, END) ? 0 : std::numeric_limits<size_t>::max();
                } else if ((ends_at(child1) == START && ends_at(child2) == END) ||
                            (ends_at(child1) == END && ends_at(child2) == START)) {
                    if (has_external_connectivity(cached_child1.tag, START, END)) {
                        //If we can take an edge around the snarl
                        return 0;
                    } else if (has_external_connectivity(cached_child1.tag, START, START) && has_external_connectivity(cached_child1.tag, END, END)) {
                        //If we can take the loops on the two ends of the snarl, walk through the snarl
                        return cached_child1.minimum_length == std::numeric_limits<size_t>::max() 
                                ? minimum_length(child1) 
                                : cached_child1.minimum_length;
                    }
                }             
            }
            return std::numeric_limits<size_t>::max();
            ...
            size_t rank1 = cached_child1.rank_in_parent == std::numeric_limits<size_t>::max() 
                            ? SnarlTreeRecord(child1, &snarl_tree_records).get_rank_in_parent()
                            : cached_child1.rank_in_praent;
            size_t rank2 = cached_child2.rank_in_parent == std::numeric_limits<size_t>::max()
                            ? SnarlTreeRecord(child2, &snarl_tree_records).get_rank_in_parent()
                            : cached_child2.rank_in_parent;
        ...
        for (size_t i = 0 ; i <=1 ; i ++ ) {
            ...
            if (is_node(cached_child.net)) {
                cached_node.set_minimum_length();
                cached_node.set_rank_in_parent();
                cached_node.set_node_values();

                bool go_left = cached_child1.is_reversed_in_parent;
                if (ends_at(cached_child.net) == START){
                    go_left = !go_left;
                }
                chain_values = std::make_tuple(cached_child.rank_in_parent, go_left, cached_child.minimum_length,
                                    cached_child.prefix_sum, cached_child.forward_loop, cached_child.reverse_loop,
                                    cached_child.chain_component);
            } else {
                //If this is a snarl, then find the appropriate boundary node and fill it in
                if (ends_at(cached_child.net) == START) {
                    net_handle_t start_bound = cached_child.start_bound_in == std::numeric_limits<size_t>::max() 
                                                   ? get_bound(cached_child.net, false, true)
                                                   : cached_child.start_bound_in;
                    size_t start_length = cached_child.start_bound_lenth == std::numeric_limits<size_t>::max() 
                                                    ? get_node_length(start_bound)
                                                    : cached_child.start_bound_length;
                    cached_child.set_start_bound(start_bound, true, start_length); 

                    chain_values = std::make_tuple(get_record_offset(cached_child.start_bound_in), true, cached_child.start_bound_length,
                                    cached_child.prefix_sum, cached_child.forward_loop, cached_child.reverse_loop,
                                    cached_child.chain_component);
                } else {
                    net_handle_t end_bound = cached_child.end_bound_in == std::numeric_limits<size_t>::max() 
                                                   ? get_bound(cached_child.net, true, true)
                                                   : cached_child.end_bound_in;
                    size_t end_length = cached_child.end_bound_lenth == std::numeric_limits<size_t>::max() 
                                                    ? get_node_length(end_bound)
                                                    : cached_child.end_bound_length;
                    cached_child.set_end_bound(end_bound, true, end_length); 

                    chain_values = std::make_tuple(get_record_offset(cached_child.end_bound_in), false, cached_child.end_bound_length,
                                    cached_child.end_prefix_sum, cached_child.end_forward_loop, cached_child.end_reverse_loop,
                                    cached_child.end_chain_component);
                }
            }
        }
        //Chain values<0> is the record offset and chain_values<1> is go_left
        if (is_node(child1) != is_node(child2) && std::get<0>(chain_values1) == std::get<0>(chain_values2) &&
                std::get<1>(chain_values1) != std::get<1>(chain_values2)) {
            ...
            return 0;
        } else if (!is_node(child1) && !is_node(child2) && std::get<0>(chain_values1) == std::get<0>(chain_values2) &&
                std::get<1>(chain_values1) != std::get<1>(chain_values2)) {
...
net_handle_t SnarlDistanceIndex::get_node_net_handle(const nid_t id) const {
    RootRecord root_record (get_root(), &snarl_tree_records);
    size_t node_pointer_offset = get_node_pointer_offset(id, root_record.get_min_node_id(), root_record.get_connected_component_count());
    return get_net_handle(snarl_tree_records->at(node_pointer_offset), START_END, NODE_HANDLE); 
}
...
bool SnarlDistanceIndex::has_external_connectivity(const size_t tag, endpoint_t start, endpoint_t end) {
...
size_t SnarlDistanceIndex::SnarlTreeRecord::get_parent_record_offset() const {
    ...
    if (type == ROOT) {
        ...
        return record_offset - ((*records)->at(record_offset + NODE_PARENT_OFFSET) >> 1);
    } else if (type == SNARL || type == DISTANCED_SNARL || type == OVERSIZED_SNARL
            || type == ROOT_SNARL || type == DISTANCED_ROOT_SNARL)  {
...
void SnarlDistanceIndex::SnarlTreeRecordConstructor::set_start_node(handlegraph::nid_t id, bool rev) {
    record_t type = get_record_type();
    size_t offset;
    if (type == ROOT || type == NODE || type == DISTANCED_NODE || type == NODE_CHAIN || type == DISTANCED_NODE_CHAIN || type == MULTICOMPONENT_NODE_CHAIN) {
...
bool SnarlDistanceIndex::NodeRecord::in_chain() const {
    return get_record_type() == NODE_CHAIN || get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN;
}

size_t SnarlDistanceIndex::NodeRecord::get_prefix_sum() const {
#ifdef debug_distances
    assert(get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
    size_t value =  (*records)->at(record_offset + NODE_PREFIX_SUM_OFFSET);
    value = value == 0 ? std::numeric_limits<size_t>::max() : value - 1;
    return value;
}
size_t SnarlDistanceIndex::NodeRecord::get_forward_loop() const {
#ifdef debug_distances
    assert(get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
...
size_t SnarlDistanceIndex::NodeRecord::get_reverse_loop() const {
#ifdef debug_distances
    assert(get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
    size_t value = (*records)->at(record_offset + NODE_REVERSE_LOOP_OFFSET);
    value = value == 0 ? std::numeric_limits<size_t>::max() : value - 1;
    return value;
}
size_t SnarlDistanceIndex::NodeRecord::get_chain_component(bool get_end) const {
#ifdef debug_distances
    assert(get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
    if (!in_chain() ||
        (!get_end && record_offset == ChainRecord(get_parent_record_offset(), records).get_first_node_offset())) {
        //If this isn't a chain, or we want the first component of the first node
        return 0;
    }
    return (*records)->at(record_offset + NODE_COMPONENT_OFFSET);
}

void SnarlDistanceIndex::NodeRecordConstructor::set_node_id(size_t value) const {
#ifdef debug_distance_indexing
...
void SnarlDistanceIndex::NodeRecordConstructor::set_prefix_sum(size_t value) const {
#ifdef debug_distance_indexing
    cerr <<record_offset + NODE_PREFIX_SUM_OFFSET << " set node prefix sum " << value << endl;
    assert((*records)->at(record_offset + NODE_PREFIX_SUM_OFFSET) == 0);
    assert(get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
    value = value == std::numeric_limits<size_t>::max() ? 0 : value +1;
    (*records)->at(record_offset + NODE_PREFIX_SUM_OFFSET) = value;
}
void SnarlDistanceIndex::NodeRecordConstructor::set_forward_loop(size_t value) const { 
#ifdef debug_distance_indexing
    cerr <<record_offset + NODE_FORWARD_LOOP_OFFSET << " set node forward loop " << value << endl;
    assert((*records)->at(record_offset + NODE_FORWARD_LOOP_OFFSET) == 0);
    assert(get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
    value = value == std::numeric_limits<size_t>::max() ? 0 : value +1;
    (*records)->at(record_offset + NODE_FORWARD_LOOP_OFFSET) = value;
}
void SnarlDistanceIndex::NodeRecordConstructor::set_reverse_loop(size_t value) const {
#ifdef debug_distance_indexing
    cerr <<record_offset + NODE_REVERSE_LOOP_OFFSET << " set node reverse loop " << value << endl;
    assert((*records)->at(record_offset + NODE_REVERSE_LOOP_OFFSET) == 0);
    assert(get_record_type() == DISTANCED_NODE_CHAIN || get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
    value = value == std::numeric_limits<size_t>::max() ? 0 : value +1;
    (*records)->at(record_offset + NODE_REVERSE_LOOP_OFFSET) = value;
}
void SnarlDistanceIndex::NodeRecordConstructor::set_chain_component(size_t value) const {
#ifdef debug_distance_indexing
    cerr <<record_offset + NODE_COMPONENT_OFFSET << " set node component " << value << endl;
    assert((*records)->at(record_offset + NODE_COMPONENT_OFFSET) == 0);
    assert(get_record_type() == MULTICOMPONENT_NODE_CHAIN);
#endif
...
size_t SnarlDistanceIndex::ChainRecord::get_distance(size_t rank1, bool left_side1, size_t node_length1, 
    ...
    if (rank1 > rank2) {
        size_t tmp_rank=rank1; bool tmp_side=left_side1; size_t tmp_len=node_lenth1;
...
size_t SnarlDistanceIndex::ChainRecord::get_distance_taking_chain_loop(size_t rank1, bool left_side1, size_t node_length1, 
...
#ifdef debug_distances
    assert (SnarlTreeRecord(rank1, records).get_rank_in_parent() <= 
        SnarlTreeRecord(rank2, records).get_rank_in_parent());
    ...
    if (get_record_handle_type() == NODE_HANDLE) {
        ...
        bool first_in_first_component = componen1 == 0 || component1 == last_component;
...
//size_t and bool are the index of the node/snarl record (within the chain) and true if it is a snarl
pair<size_t, bool> SnarlDistanceIndex::ChainRecord::get_next_child(const pair<size_t, bool> pointer, bool go_left) const {
    ...
    size_t node_record_size = get_record_type() == MULTICOMPONENT_CHAIN ? NODE_MULTICOMPONENT_RECORD_SIZE : NODE_CHAIN_RECORD_SIZE;
    if (pointer.second) {
        //This is a snarl
        if (go_left) {
            return make_pair(pointer.first - node_record_size - 1, false);
        } else {
        ...
        if (go_left) {
            ...
            if ((*records)->at(pointer.first+node_record_size) == 0 &&
                (*records)->at(pointer.first+node_record_size+1) == 0) {
            ...
            size_t snarl_record_size = (*records)->at(pointer.first+node_record_size);
            return make_pair(pointer.first+node_record_size+1, snarl_record_size != 0);
        }
    }
}
...
bool SnarlDistanceIndex::ChainRecord::for_each_child(const std::function<bool(const net_handle_t&)>& iteratee) const {
    ...
    while (current_child.first != 0) {
        net_handle_t child_handle = current_child.second 
            ...
            : get_net_handle (current_child.first, START_END, NODE_HANDLE);
...
SnarlDistanceIndex::NodeRecordConstructor SnarlDistanceIndex::ChainRecordConstructor::add_node(nid_t node_id) {
    size_t start_i = (*records)->size();
    bool is_multicomponent = get_record_type() == MULTICOMPONENT_CHAIN;
    return NodeRecordConstructor(start_i, is_multicomponent ? MULTICOMPONENT_NODE_CHAIN : DISTANCED_NODE_CHAIN, records, node_id); 
}
...
#ifdef debug_distance_indexing
    cerr << (*records)->size() << "  Adding chain's trivial snarl to the end of the array " << endl;
#endif
    size_t start_i = (*records)->size();
    (*records)->resize(start_i+1);
        ...
        while (!temp_record_stack.empty()) {
            ...
            if (current_record_index.first == TEMP_CHAIN) {
                ...
                if (!temp_chain_record.is_trivial) {
                    ...
                    for (size_t child_record_index_i = 0 ; child_record_index_i < temp_chain_record.children.size() ; child_record_index_i++) {
                        ...
                        if (child_record_index.first == TEMP_NODE) {
                            ...
                            cerr << "    This is a looping chain"  << endl;
                                ...
                                last_child_offset = make_pair(snarl_tree_records->size(), false);



                                //Make a new node record
                                NodeRecordConstructor node_record_constructor = chain_record_constructor.add_node(temp_node_record.node_id);
                                //Fill in this node in the index 
                                node_record_constructor.set_node_id(temp_node_record.node_id);
                                ...
                                node_record_constructor.set_prefix_sum(temp_chain_record.prefix_sum[chain_node_i]);
                                node_record_constructor.set_forward_loop(temp_chain_record.forward_loops[chain_node_i]);
                                node_record_constructor.set_reverse_loop(temp_chain_record.backward_loops[chain_node_i]);

                                if (chain_record_constructor.ChainRecord::get_record_type() == MULTICOMPONENT_CHAIN) {
                                    node_record_constructor.set_chain_component(temp_chain_record.chain_components[chain_node_i]);
                                }


#ifdef debug_distance_indexing
                            cerr << "    The node record is at offset " << node_record_constructor.record_offset << endl;
#endif
                            }
                    ...
                    NodeRecordConstructor node_record(snarl_tree_records->size(), record_type, &snarl_tree_records, temp_node_record.node_id);
